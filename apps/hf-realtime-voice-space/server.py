...
Endpoints:
  ...
  POST /api/memory/recall    -> deterministic MCP memory recall facade
  POST /api/memory/remember  -> deterministic MCP memory write/upsert facade
...
MEMORY_RECALL_MAX_QUERIES = _bounded_int_env("MEMORY_RECALL_MAX_QUERIES", 10, 3, 20)
MEMORY_RECALL_MAX_ENTITIES = _bounded_int_env("MEMORY_RECALL_MAX_ENTITIES", 12, 1, 30)
MEMORY_RECALL_MAX_OBSERVATIONS = _bounded_int_env("MEMORY_RECALL_MAX_OBSERVATIONS", 8, 1, 30)
MEMORY_DEFAULT_RECALL_QUERIES = [
    item.strip()
    for item in os.environ.get(
        "MEMORY_DEFAULT_RECALL_QUERIES",
        "User,Lazar,Лазар,Lazar Mateev,Лазар Матеев,Mateevi family,Семейство Матееви,Mateevi,Матееви",
    ).split(",")
    if item.strip()
]
MEMORY_ALIAS_GROUPS = (
    ("User", "Lazar", "Лазар", "Lazar Mateev", "Лазар Матеев"),
    ("Mateevi family", "Семейство Матееви", "Mateevi", "Матееви", "family", "семейство"),
    ("Plovdiv", "Пловдив"),
    ("Desi Mateeva", "Деси Матеева", "Desi", "Деси"),
    ("Viktoria", "Виктория"),
    ("Kristian", "Кристиан"),
)
...
def _memory_tokens(text: str) -> list[str]:
    tokens = re.findall(r"[A-Za-zА-Яа-яЁё0-9][A-Za-zА-Яа-яЁё0-9._-]{1,}", text or "")
    return _dedupe_strings([token for token in tokens if len(token) >= 2])


def _memory_aliases_for_text(text: str) -> list[str]:
    haystack = _memory_norm(text)
    aliases: list[str] = []
    for group in MEMORY_ALIAS_GROUPS:
        group_norms = {_memory_norm(item) for item in group}
        if any(item and item in haystack for item in group_norms):
...
def _memory_recall_queries(
    ...
    chunks = [str(query or "").strip()]
    chunks.extend(_string_list(hints))
    if language:
        chunks.append(str(language))
    combined = " ".join(chunks)
    combined_norm = _memory_norm(combined)

    candidates: list[str] = []
    if any(trigger in combined_norm for trigger in MEMORY_RECALL_TRIGGERS):
        candidates.extend(MEMORY_DEFAULT_RECALL_QUERIES)

    for hint in _string_list(hints):
        candidates.append(hint)
        candidates.extend(_memory_aliases_for_text(hint))

    candidates.extend(_memory_aliases_for_text(combined))
    candidates.extend(_memory_tokens(combined))

    # Avoid sending the full natural-language sentence to the graph search. The
    # memory server performs simple matching; compact entity-like queries work
    # much better than one long mixed-language phrase.
    compact = [item for item in _dedupe_strings(candidates) if len(item) <= 80]
    limit = max(1, min(max_queries or MEMORY_RECALL_MAX_QUERIES, 20))
    return compact[:limit]


def _memory_open_names_for_queries(queries: list[str]) -> list[str]:
    names: list[str] = []
    for query in queries:
        names.append(query)
        names.extend(_memory_aliases_for_text(query))
    return _dedupe_strings(names)[:24]


def _mcp_text_payload(result: object) -> str:
...
def _collect_memory_graph(results: list[dict]) -> tuple[list[dict[str, object]], list[dict[str, str]]]:
    entities_by_name: dict[str, dict[str, object]] = {}
    relations_by_key: dict[tuple[str, str, str], dict[str, str]] = {}

    def add_entity(raw: object) -> None:
        if not isinstance(raw, dict):
            return
        name = str(raw.get("name") or raw.get("entityName") or "").strip()
        if not name:
            return
        key = _memory_norm(name)
        entity = entities_by_name.setdefault(
            key,
            {
                "name": name,
                "entityType": str(raw.get("entityType") or raw.get("type") or "memory").strip() or "memory",
                "observations": [],
            },
        )
        observations = entity.setdefault("observations", [])
        if isinstance(observations, list):
            observations.extend(_string_list(raw.get("observations") or raw.get("contents") or raw.get("notes")))
            observations.extend(_memory_property_observations(raw.get("properties")))
            entity["observations"] = _dedupe_strings(observations)[:MEMORY_RECALL_MAX_OBSERVATIONS]

    def add_relation(raw: object) -> None:
        if not isinstance(raw, dict):
            return
        source = str(raw.get("from") or raw.get("source") or "").strip()
        target = str(raw.get("to") or raw.get("target") or "").strip()
        relation_type = str(raw.get("relationType") or raw.get("relation") or raw.get("type") or "").strip()
        if not source or not target or not relation_type:
            return
        key = (_memory_norm(source), _memory_norm(relation_type), _memory_norm(target))
        relations_by_key[key] = {"from": source, "relationType": relation_type, "to": target}

    for result in results:
        payload = _mcp_json_payload(result)
        if isinstance(payload, dict):
            ...
            for item in payload:
                if isinstance(item, dict) and ("from" in item or "to" in item or "relationType" in item):
                    add_relation(item)
                else:
                    add_entity(item)

    return list(entities_by_name.values()), list(relations_by_key.values())


def _rank_memory_entities(entities: list[dict[str, object]], queries: list[str]) -> list[dict[str, object]]:
    query_norms = [_memory_norm(query) for query in queries if query]

    def score(entity: dict[str, object]) -> tuple[int, str]:
        name = str(entity.get("name") or "")
        name_norm = _memory_norm(name)
        observations = " ".join(_string_list(entity.get("observations")))
        obs_norm = _memory_norm(observations)
        value = 0
        for query_norm in query_norms:
            ...
            if query_norm == name_norm:
                ...
                value += 10
            elif query_norm in obs_norm:
                value += 2
        return (-value, name_norm)

    return sorted(entities, key=score)[:MEMORY_RECALL_MAX_ENTITIES]


def _memory_summary(entities: list[dict[str, object]], relations: list[dict[str, str]]) -> str:
    lines: list[str] = []
    for entity in entities[:6]:
        name = str(entity.get("name") or "").strip()
        observations = _string_list(entity.get("observations"))
        if name and observations:
            lines.append(f"{name}: {'; '.join(observations[:3])}")
...
def _existing_memory_observations(entities: list[dict[str, object]]) -> dict[str, set[str]]:
    existing: dict[str, set[str]] = {}
    for entity in entities:
        name = str(entity.get("name") or "").strip()
        if not name:
            continue
        existing[_memory_norm(name)] = {_memory_norm(item) for item in _string_list(entity.get("observations"))}
    return existing


def _memory_tool_params(name: str, arguments: dict) -> tuple[str, dict]:
    return "tools/call", {"name": name, "arguments": _clean_mcp_arguments(name, arguments)}
...
async def memory_recall(req: MemoryRecallRequest):
    """Recall persistent graph memory with deterministic query expansion."""
    if "search_nodes" not in MCP_ALLOWED_TOOLS:
        return {
            "found": False,
            "error": "MCP memory search_nodes is not allowlisted.",
            "queriesTried": [],
            "entities": [],
            "relations": [],
            "summary": "",
    ...
    queries = _memory_recall_queries(req.query, req.hints, req.language, req.maxQueries)
    if not queries:
        return {
            "found": False,
            "error": "No usable memory recall queries were generated.",
            "queriesTried": [],
            "entities": [],
            "relations": [],
            "summary": "",
    ...
    calls = [_memory_tool_params("search_nodes", {"query": query}) for query in queries]
    open_names = _memory_open_names_for_queries(queries)
    if open_names and "open_nodes" in MCP_ALLOWED_TOOLS:
        calls.append(_memory_tool_params("open_nodes", {"names": open_names}))
    ...
    entities, relations = _collect_memory_graph(results)
    ranked_entities = _rank_memory_entities(entities, queries)
    summary = _memory_summary(ranked_entities, relations)
    return {
        "found": bool(ranked_entities or relations),
        "queriesTried": queries,
        "openNamesTried": open_names,
...
async def memory_remember(req: MemoryRememberRequest):
    """Create/update persistent graph memory and verify the write."""
    if not {"open_nodes", "create_entities", "add_observations"}.issubset(MCP_ALLOWED_TOOLS):
        return {
            "ok": False,
            "error": "MCP memory write tools are not allowlisted.",
            "createdEntities": [],
            "addedObservations": [],
    ...
    for raw_entity in req.entities or []:
        entity = _normalize_memory_entity(raw_entity)
        key = _memory_norm(entity["name"])
        existing = desired_by_name.setdefault(
            key,
            {"name": entity["name"], "entityType": entity["entityType"], "observations": []},
        )
        existing["entityType"] = entity["entityType"] or existing.get("entityType") or "memory"
        existing["observations"] = _dedupe_strings(
            _string_list(existing.get("observations")) + _string_list(entity.get("observations"))
        )

    for raw_observation in req.observations or []:
        observation = _normalize_memory_observation(raw_observation)
        key = _memory_norm(observation["entityName"])
        existing = desired_by_name.setdefault(
            key,
            {"name": observation["entityName"], "entityType": "memory", "observations": []},
        )
        existing["observations"] = _dedupe_strings(
            _string_list(existing.get("observations")) + _string_list(observation.get("contents"))
        )

    relations = [_normalize_memory_relation(relation) for relation in req.relations or []]
    affected_names = _dedupe_strings(
        [str(item.get("name") or "") for item in desired_by_name.values()]
    ...
    if not affected_names:
        return {
            "ok": False,
            "error": "No memory entities, observations, or relations were provided.",
            "createdEntities": [],
            "addedObservations": [],
            "skippedDuplicateObservations": [],
            "relationsRequested": [],
    ...
    try:
        existing_results = await _mcp_requests([_memory_tool_params("open_nodes", {"names": affected_names})])
    except HTTPException as exc:
        return {
            "ok": False,
            "error": _safe_detail(exc.detail),
            ...
            "addedObservations": [],
            "skippedDuplicateObservations": [],
            "relationsRequested": relations,
            "verified": {},
        }

    existing_entities, _existing_relations = _collect_memory_graph(existing_results)
    existing_by_norm = {_memory_norm(entity.get("name")): entity for entity in existing_entities}
    existing_observations = _existing_memory_observations(existing_entities)

    entities_to_create: list[dict[str, object]] = []
    observations_to_add: list[dict[str, object]] = []
    skipped_duplicates: list[dict[str, object]] = []

    for key, desired in desired_by_name.items():
        desired_name = str(desired.get("name") or "").strip()
        desired_observations = _string_list(desired.get("observations"))
        if key not in existing_by_norm:
            entities_to_create.append(
                {
                    "name": desired_name,
                    "entityType": str(desired.get("entityType") or "memory"),
                    "observations": desired_observations,
                }
            )
            continue

        existing_entity = existing_by_norm[key]
        existing_name = str(existing_entity.get("name") or desired_name)
        existing_obs = existing_observations.get(key, set())
        new_observations = []
        duplicate_observations = []
        for observation in desired_observations:
            if _memory_norm(observation) in existing_obs:
                duplicate_observations.append(observation)
            else:
                new_observations.append(observation)
        if new_observations:
            observations_to_add.append({"entityName": existing_name, "contents": new_observations})
        if duplicate_observations:
            skipped_duplicates.append({"entityName": existing_name, "contents": duplicate_observations})

    known_names = {_memory_norm(name) for name in affected_names}
    known_names.update(existing_by_norm.keys())
    known_names.update(_memory_norm(entity["name"]) for entity in entities_to_create)
    relations_to_create = [
        relation
        for relation in relations
        if _memory_norm(relation["from"]) in known_names and _memory_norm(relation["to"]) in known_names
    ]

    write_calls: list[tuple[str, dict | None]] = []
    if entities_to_create:
        write_calls.append(_memory_tool_params("create_entities", {"entities": entities_to_create}))
    if observations_to_add:
        write_calls.append(_memory_tool_params("add_observations", {"observations": observations_to_add}))
    if relations_to_create and "create_relations" in MCP_ALLOWED_TOOLS:
        write_calls.append(_memory_tool_params("create_relations", {"relations": relations_to_create}))
    if req.verify:
        write_calls.append(_memory_tool_params("open_nodes", {"names": affected_names}))

    try:
        write_results = await _mcp_requests(write_calls) if write_calls else []
    except HTTPException as exc:
        return {
            "ok": False,
            "error": _safe_detail(exc.detail),
            "createdEntities": entities_to_create,
            "addedObservations": observations_to_add,
            "skippedDuplicateObservations": skipped_duplicates,
            "relationsRequested": relations_to_create,
            "verified": {},
        }

    verified: dict[str, object] = {}
    if req.verify and write_results:
        verified_entities, verified_relations = _collect_memory_graph([write_results[-1]])
        verified = {
