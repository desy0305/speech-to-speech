...
def parse_arguments() -> ParsedArguments:
    ...
    return ParsedArguments(
        ...
        ani_voice_tts_handler_kwargs=by_type[AniVoiceTTSHandlerArguments],
    )
...
def prepare_all_args(
    ...
    rename_args(ani_voice_tts_handler_kwargs, "ani_voice")
...
def _build_realtime_pipeline_unit(
    *,
    ...
    ani_voice_tts_kw = deepcopy(ani_voice_tts_handler_kwargs)
    ...
    handlers = _build_pipeline_handlers(
        ...
        ani_voice_tts_handler_kwargs=ani_voice_tts_kw,
        speculative_turns=speculative_turns,
    )
    for h in handlers:
    ...
    if module_kwargs.mode == "local":
        ...
        pool = [
            _build_realtime_pipeline_unit(
                ...
                ani_voice_tts_handler_kwargs=ani_voice_tts_handler_kwargs,
            )
...
def get_tts_handler(
    ...
    elif module_kwargs.tts == "ani-voice":
        from speech_to_speech.TTS.ani_voice_tts_handler import AniVoiceTTSHandler

        return AniVoiceTTSHandler(
            stop_event,
            queue_in=lm_response_queue,
            queue_out=send_audio_chunks_queue,
            setup_args=(should_listen,),
            setup_kwargs=vars(ani_voice_tts_handler_kwargs),
        )
    else:
        raise ValueError("The TTS should be either chatTTS, facebookMMS, pocket, kokoro, qwen3, or ani-voice")
...
def main() -> None:
    ...
    prepare_all_args(
        ...
        args.ani_voice_tts_handler_kwargs,
    )
